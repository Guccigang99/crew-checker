...
st.write("Upload je crew-database en je Strobbo weekrooster. De app markeert fouten ook op een kopie van je Strobbo Excel.")
...
def zoek_beste_match(strobbo_naam, crew_df):
    ...
    # Exacte voornaam, bv. Baue -> Baue Segaert
...
def parse_shiftblok(celtekst, datum):
    """Leest 1 cel. 1 cel = meestal 1 taakblok, soms meerdere. Pauze is de tekst tussen haakjes."""
    ...
    for m in pattern.finditer(s):
        ...
        blokken.append({
            "start": start,
            "einde": einde,
            "pauze_minuten": pauze_min,
        })
    return blokken


def merge_blokken_naar_shifts(blokken):
    """Aansluitende taakblokken worden 1 echte shift. Een echte onderbreking blijft aparte shift."""
    if not blokken:
        return []
...
for _, sh in shifts_df.dropna(subset=["database_naam"]).iterrows():
    ...
    if netto < MIN_DAGUREN:
        fout_toevoegen(
            ...
            f"{netto:.2f}u gewerkt. Minimum is {MIN_DAGUREN}u.",
            cellen, "Shift verlengen of verwijderen"
        )
    ...
    if leeftijd is not None and leeftijd < 18:
        if netto > MAX_DAGUREN_MINDERJARIG:
            fout_toevoegen(
                ...
                f"{netto:.2f}u gewerkt. Maximum voor <18 is {MAX_DAGUREN_MINDERJARIG}u.",
                cellen, "Shift inkorten"
            )
    else:
        if netto > MAX_DAGUREN_VOLWASSEN:
            fout_toevoegen(
                fouten, naam, datum, "Shift te lang",
                f"{netto:.2f}u gewerkt. Maximum is {MAX_DAGUREN_VOLWASSEN}u.",
                cellen, "Shift inkorten"
            )
    ...
    if leeftijd is None or leeftijd >= 18:
        if bruto <= 5 and pauze > 0:
            fout_toevoegen(
                ...
                f"{bruto:.2f}u shift heeft {pauze} min pauze, maar tot 5u is geen pauze nodig.",
                cellen, "Hier pauze verwijderen", "Waarschuwing"
            )
        elif 5 < bruto <= 8 and pauze < 20:
            fout_toevoegen(
                fouten, naam, datum, "Pauze ontbreekt",
                f"{bruto:.2f}u shift. Minstens 20 min pauze nodig. Gepland: {pauze} min.",
                cellen, "HIER 20 min pauze toedienen"
            )
        elif bruto > 8 and pauze < 30:
            fout_toevoegen(
                fouten, naam, datum, "Pauze ontbreekt",
                f"{bruto:.2f}u shift. Minstens 30 min pauze nodig. Gepland: {pauze} min.",
                cellen, "HIER 30 min pauze toedienen"
            )

    # Pauzes minderjarigen
    if leeftijd is not None and leeftijd < 18:
        if bruto > 4.5 and bruto <= 6 and pauze < 30:
            fout_toevoegen(
                fouten, naam, datum, "Pauze minderjarige ontbreekt",
                f"{bruto:.2f}u shift. Minderjarige heeft minstens 30 min pauze nodig. Gepland: {pauze} min.",
                cellen, "HIER 30 min pauze toedienen"
            )
        elif bruto > 6 and pauze < 60:
            fout_toevoegen(
                fouten, naam, datum, "Pauze minderjarige ontbreekt",
                f"{bruto:.2f}u shift. Minderjarige heeft minstens 60 min pauze nodig. Gepland: {pauze} min.",
                cellen, "HIER 60 min pauze toedienen"
            )
    ...
    if leeftijd is not None:
        ...
        if leeftijd <= 15 and einduur > 20:
            fout_toevoegen(
                ...
                f"{leeftijd} jaar en werkt tot {sh['einde'].strftime('%H:%M')}. Max tot 20:00.",
                cellen, "Shift vroeger laten eindigen"
            )
        elif 16 <= leeftijd < 18 and einduur > 23:
            fout_toevoegen(
                fouten, naam, datum, "Nachtwerk minderjarige",
                f"{leeftijd} jaar en werkt tot {sh['einde'].strftime('%H:%M')}. Max tot 23:00.",
                cellen, "Shift vroeger laten eindigen"
            )
...
for naam, groep in shifts_df.dropna(subset=["database_naam"]).groupby("database_naam"):
    ...
    if leeftijd is not None and leeftijd < 18 and totaal > MAX_WEEKUREN_MINDERJARIG:
        fout_toevoegen(
            ...
            f"{totaal:.2f}u gewerkt. Maximum voor <18 is {MAX_WEEKUREN_MINDERJARIG}u.",
            alle_cellen, "Weekplanning verminderen"
        )
    elif (leeftijd is None or leeftijd >= 18) and totaal > MAX_WEEKUREN_VOLWASSEN:
        fout_toevoegen(
            fouten, naam, "", "Weekuren overschreden",
            f"{totaal:.2f}u gewerkt. Maximum is {MAX_WEEKUREN_VOLWASSEN}u.",
            alle_cellen, "Weekplanning verminderen"
        )
    ...
    if medewerker_type == "vast" and contracturen > 0 and totaal < contracturen:
        fout_toevoegen(
            ...
            f"{totaal:.2f}u gepland, contract is {contracturen:.2f}u. Tekort: {contracturen - totaal:.2f}u.",
            alle_cellen, "Extra uren plannen"
        )
...
for naam, groep in shifts_df.dropna(subset=["database_naam"]).groupby("database_naam"):
    ...
    for i in range(1, len(groep)):
        ...
        if rust < MIN_RUSTUREN_TUSSEN_SHIFTS:
            fout_toevoegen(
                fouten, naam, huidige["datum"], "Te weinig rust",
                f"Slechts {rust:.2f}u rust tussen {vorige['einde'].strftime('%d/%m %H:%M')} en {huidige['start'].strftime('%d/%m %H:%M')}.",
                huidige["bron_cellen"], "Rusttijd corrigeren"
            )
...
# =========================
# STROBBO EXCEL MARKEREN
# =========================

max_col = ws.max_column
opmerking_start_col = max_col + 1
ws.cell(row=1, column=opmerking_start_col).value = "Opmerking controle"
ws.cell(row=1, column=opmerking_start_col + 1).value = "Actie"
ws.cell(row=1, column=opmerking_start_col).fill = FILL_OPMERKING
ws.cell(row=1, column=opmerking_start_col + 1).fill = FILL_FOUT
ws.cell(row=1, column=opmerking_start_col).font = Font(bold=True)
ws.cell(row=1, column=opmerking_start_col + 1).font = Font(bold=True)
...
if not fouten_df.empty:
    for _, f in fouten_df.iterrows():
        ...
        tekst_fout = f"{f['Medewerker']}: {f['Fout']} - {actie}"

        for cel in cellen:
            try:
                rr, cc = cel
                ws.cell(row=rr, column=cc).fill = FILL_FOUT
                ws.cell(row=rr, column=cc).font = FONT_ROOD
                ws.cell(row=rr, column=cc).comment = Comment(str(f["Detail"]), "Strobbo Checker")
                rij_opmerkingen.setdefault(rr, []).append(tekst_fout)
            except Exception:
                pass

for rr, meldingen in rij_opmerkingen.items():
    uniek = []
    for m in meldingen:
        if m not in uniek:
            uniek.append(m)

    ws.cell(row=rr, column=opmerking_start_col).value = " | ".join(uniek)
    ws.cell(row=rr, column=opmerking_start_col).fill = FILL_OPMERKING
    ws.cell(row=rr, column=opmerking_start_col).alignment = Alignment(wrap_text=True, vertical="top")
    ...
    ws.cell(row=rr, column=opmerking_start_col + 1).value = " | ".join(acties)
    ws.cell(row=rr, column=opmerking_start_col + 1).fill = FILL_FOUT
    ws.cell(row=rr, column=opmerking_start_col + 1).alignment = Alignment(wrap_text=True, vertical="top")

ws.column_dimensions[ws.cell(row=1, column=opmerking_start_col).column_letter].width = 45
ws.column_dimensions[ws.cell(row=1, column=opmerking_start_col + 1).column_letter].width = 32
...
st.dataframe(
    toon[["strobbo_naam", "database_naam", "datum", "Start", "Einde", "pauze_minuten", "Bruto uren", "Netto uren"]],
...
if fouten_df.empty:
    ...
    zichtbare_fouten = fouten_df.drop(columns=["Cellen"], errors="ignore")
    st.dataframe(zichtbare_fouten, use_container_width=True)
...
with pd.ExcelWriter(rapport_buffer, engine="openpyxl") as writer:
    ...
    if fouten_df.empty:
        pd.DataFrame(columns=["Medewerker", "Datum", "Ernst", "Fout", "Detail", "Actie voor Excel"]).to_excel(writer, index=False, sheet_name="Foutenrapport")
    else:
