...
st.write("Upload je crew-database en je Strobbo weekrooster. De app controleert regels en markeert fouten in een kopie van je Strobbo Excel.")
...
st.dataframe(toon_shifts[["strobbo_naam", "database_naam", "datum", "Start", "Einde", "pauze_minuten", "Bruto uren", "Netto uren", "bron_rij", "bron_kolom"]], use_container_width=True)
...
if fouten_df.empty:
    ...
    st.dataframe(fouten_df.drop(columns=["bron_cellen"], errors="ignore"), use_container_width=True)
    buffer = BytesIO()
    ...
    st.download_button("📥 Download foutenrapport als Excel", data=buffer.getvalue(), file_name="foutenrapport_strobbo.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")

st.download_button("📥 Download gemarkeerde Strobbo Excel", data=annotated_buffer.getvalue(), file_name="strobbo_rooster_gecontroleerd.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
